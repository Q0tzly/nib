...
- **Ctrl-q はコアの予約キー**で、プラグインには渡さない。押すと、コアがプラグインを介さずに救済メニューを出す。
  - `w` 全部保存して終了 / `q` 保存せずに終了 / `r` 全プラグインを再起動（無効にしたものも含む） / それ以外 戻る
  - キーマップが壊れていても、キーを飲み込むバグのプラグインがいても、必ず抜け出せるようにするため。OS の Ctrl-Alt-Del と同じく、アプリ（プラグイン）には横取りさせない。
  - いきなり終了せずメニューをはさむのは、うっかり押して未保存の変更を失わないようにするため。
  - Ctrl-q にしたのは、Helix の既定の配置で使われておらず、JIS 配列でも打ちやすいから（Ctrl-\ は JIS 配列で打ちにくい）。
...
- プラグインが止まったり、落ちずにキーを飲み込み続けたりしても抜け出せるよう、コアは予約キー **Ctrl-q** を持つ（後述の「入力」）。
