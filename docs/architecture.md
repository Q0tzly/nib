...
- 文法とハイライトのクエリは、そのファイル形式を初めて表示したときに読み込む（キャッシュがあっても約 17 ms かかるため）。ファイルを開いた直後の解析も最初の画面のあとに回し、まず色なしで表示してから色を付ける。打鍵のあとの差分での解析はその場で行う。
...
- 設定ファイルは `~/.config/nib/config.toml` の 1 つ。`[core]` にコアの設定を、`[plugins.<name>]` にプラグインごとの設定を、同じ書き方で書く。
- プラグインごとの設定は、コアが解釈せずにそのままプラグインへ渡す。
- コアの設定は 2 種類に分ける。

| 種類 | 例 | 変えられるのは |
|------|----|----------------|
| 編集の振る舞い | タブ幅、インデント、カーソルの上下に残す行数、テーマ | 設定ファイルとプラグイン |
| 安全装置 | プラグインの時間とメモリの上限、権限、予約キー、読み込むプラグインの一覧 | 設定ファイル（利用者）だけ |
...
- 値は「設定ファイルの全体の値」を「プラグインがバッファ単位で上書きする」の 2 段だけにする。言語ごとの既定値のような段はコアに持たず、プラグインの仕事にする。
...
- 「コアがそのプラグインをどう動かすか」は、プラグインごとに `[core.plugin.<name>]` に集める。プラグイン自身に渡す設定（`[plugins.<name>]`）とは分ける。どちらも利用者だけが書き、プラグインからは変えられない。

```toml
[core]
plugin-dirs = ["~/dev/my-plugin"]   # 埋め込み以外に読み込むプラグイン

[core.plugin.lsp]
load = "lazy"          # "startup" か "lazy"（マニフェストが宣言した条件で読み込む）
timeout-ms = "none"    # 呼び出しの時間制限。数値か "none"
memory-mib = 512
enabled = false

[plugins.lsp]          # lsp プラグインに渡す設定
server = "rust-analyzer"
```

  - `timeout-ms = "none"` は、Ctrl-g で処理中のプラグインを止められるようになってから（M3）受け付ける。それまでは、無限ループに入ると抜け出す手段がないため。遅い呼び出しの記録は、制限がなくても続ける。
  - コアメニューのプラグイン一覧に、読み込み方と上限も出す。
  - 言語（文法とクエリ）は設定によらず、そのファイル形式を初めて表示したときに読み込む。
- インストールと更新（git などからの取得、バージョンの解決、更新）はコアに入れない。ネットワークとファイル操作を伴う、エディタ本体とは別の複雑さなので、本体の外で `nib plugin add <url>` のようなサブコマンドとして作る。
