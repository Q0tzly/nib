...
- tree-sitter の解析はコアが行う。編集のたびに、トランザクションの変更範囲を使って差分で解析し直す。
  ...
  - undo と redo も、変更前と変更後のテキストの共通の先頭と末尾から、全体を覆う 1 つの編集として伝える。構文木を捨てると、解析し終わるまで色が消えるため。
...
- 文法とハイライトのクエリは、そのファイル形式を初めて表示したときに読み込む（キャッシュがあっても約 17 ms かかるため）。クエリのコンパイル（Rust のハイライトで 13 ms。tree-sitter はコンパイルしたクエリを保存できない）は別のスレッドで行い、最初の解析と並べる。ファイルを開いた直後の解析も最初の画面のあとに回し、まず色なしで表示してから色を付ける。打鍵のあとの解析は、次の「解析のスレッド」で行う。
...
- プラグインは API を通じて、構文ノードの検索やクエリの実行ができる（[plugin-api.md](plugin-api.md) の「構文木」）。テキストオブジェクト（「関数を選択」など）はこれで実現する。
  - プラグインが構文木を読むときは、解析を最新にしてから答える（裏で解析中なら、それが終わるのを待つ）。プラグインは解析の時機を気にしなくてよい。ただし、打鍵のたびに読むと、そのたびに解析を待つことになる。打鍵のたびに要るもの（括弧の強調など）は、`editor.syntax_updated` イベント（[plugin-api.md](plugin-api.md) の「イベント」）を受けてから読む。

### 解析のスレッド

打鍵から画面までの時間のうち、構文木の解析がいちばん大きい（3,455 行の Rust ファイルで、打つ場所によって 0.05〜1.3 ms。[benchmarks.md](benchmarks.md)）。解析を専用のスレッドで行い、打鍵ではそれを待たずに描く。

- 編集では、構文木に編集を伝える（`Tree::edit`）だけにする。位置をずらした古い構文木で色を付けて描き、解析はスレッドに頼む。1 つのバッファにつき、頼む解析は同時に 1 つ。
  - 古い構文木で描くと、編集した場所のまわりの色が、解析が終わるまでの一瞬だけ前のままになる（たとえば `//` を打った行が、コメントの色になるのが 1 ms ほど遅れる）。
- 解析が終わると、スレッドはメインループを起こす（外部プロセスの出力と同じ仕組み）。メインループは、構文木を差し替え、injection の層を更新して描き直し、`editor.syntax_updated` を出す。
  - 解析のあいだに次の編集が来ていたら、結果の構文木にその編集を当て直し、それを古い構文木として、もう一度頼む。解析を途中で止めることはしない。
- スレッドは、自分の解析器と、WASM の文法のストアを持つ。文法（`Language`）とエンジンはメインスレッドと共有し、スレッドのストアには初めて使うときに文法を読み込む。メインスレッドは、文法の読み込み、injection の層の解析、構文木の API に答えるときの解析のために、別の解析器を持つ。
- 構文木の API に答えるときは、頼んでいる解析が終わるのを待ち、それでも古ければ（待っているあいだに編集が来たなど）メインスレッドで解析する。
- 裏で解析するかどうかは、フロントエンドが選ぶ。tui は裏で解析する。選ばなければ、今までどおり打鍵のあとにその場で解析する。テストと `nib plugin test` が、いつも同じ結果を得るため。
- injection の層の解析は、当面メインスレッドに残す（層は小さく、差分で解析するので速い）。重くなったら同じスレッドに移す。
