...
> コードブロックとドキュメントコメントは完了（2026-09-26）。3,455 行の Rust ファイル（ドキュメントコメント 592 行、Apple M1 Pro）で、打鍵は中央値 0.81 ms / p99 2.78 ms（injection なしで 0.73 / 2.30 ms）、半画面スクロールは差がばらつきの範囲に収まった。ドキュメントコメントの Markdown に色が付くまでは起動から 61 ms（なしで 46 ms。Rust の色はこれまでどおり先に付く）。
>
> 計画から変えた点:
> - injection のクエリは、打鍵のたびに構文木の全体へ掛けると 3 ms かかった。前回の結果を覚えておき、構文木が変わった範囲と編集した範囲にだけ掛け直す。
...
- 残り: Markdown の行内の要素（強調、コード、リンク）。行内の文法（`tree-sitter-markdown_inline.wasm`）を足す

確かめ方: nib のリポジトリの `docs/*.md` のコードブロックと、`core/src/*.rs` のドキュメントコメントに色が付くことを見る。
